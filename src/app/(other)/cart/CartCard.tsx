...
const CartCard = ({ cart }: props) => {
  ...
  return (
    <Card className="max-w-sm w-full h-full">
      <CardHeader className="border-b-2 border-lblack">
        PRICE DETAILS
      </CardHeader>
      {cart && (
        <CardContent className="w-full mt-5 text-sm">
          ...
          <div className="w-full flex flex-row my-4 border-y-2 text-xl border-lblack">
      ...
      <CardFooter>
        ...
        <Link
          href={"makeOrder"}
          className="w-full text-center my-2 py-4 rounded-sm bg-black text-white"
        >
          Proceed to Address
        </Link>
      </CardFooter>
